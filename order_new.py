...
ORDER_IDS = [3851,3855]  # список заказов
...
inventory_path = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQDLvIlEqbNSAGQ7qBSbvy-S9MdYE-ENSRuVKGjHaVurGR9qcY1_jZMAGCkSrDMpd14Nwbp8ud3ihjx/pub?output=csv"
...
for col in ['price', 'brand', 'in_box', 'box_price', 'moq', 'my_price', 'sklad']:
    if col not in inventory.columns:
        inventory[col] = '-'

# --- Преобразуем my_price в число ---
inventory['my_price'] = pd.to_numeric(inventory['my_price'], errors='coerce').fillna(0).astype(float)

# --- Создаем лист для закупочного инвойса ---
ws = wb.create_sheet("Purchase Invoice")
excel_headers = [
    "Order ID", "Баркод", "Название", "Артикул", "Количество",
    "Фактический остаток", "Нужно заказать", "Статус", "Склад",
    "Box price ($)", "price ($)", "my_price", "moq", "brand", "in box"
]
...
for oid in ORDER_IDS:
    ...
    df = pd.merge(
        order_df,
        inventory[['barcode', 'code', 'in stock', 'category', 'price', 'my_price', 'brand', 'in_box', 'box_price', 'moq', 'sklad']],
    ...
    for col in ['box_price', 'price', 'my_price', 'moq']:
    ...
    df['my_price'] = df['my_price'].apply(lambda x: round(x, 2) if pd.notna(x) else '-')
    df['moq'] = df['moq'].apply(lambda x: int(x) if pd.notna(x) else '-')
    df['brand'] = df['brand'].fillna('-')
    df['in box'] = df['in_box'].fillna('-')
    df['sklad'] = df['sklad'].fillna('-')
    ...
    for _, row in df.iterrows():
        ws.append([
            ...
            row['in stock'], row['Нужно заказать'], row['Статус'], row['sklad'],
            row['Box price ($)'], row['price ($)'], row['my_price'], row['moq'], row['brand'], row['in box']
        ])
